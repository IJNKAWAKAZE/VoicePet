...
- 中文唤醒词默认“你好，小蓝”，可在设置中修改为 2 到 12 个中文字符并调整灵敏度
- 基于 Sherpa-ONNX 的本地关键词检测，模型缺失时在设置页提示并支持断点续传下载
- 连续对话：唤醒后可在设定的等待时间内连续说话，不必重复唤醒词
- 基于 faster-whisper 的本地语音识别，可选择 tiny / base / small / medium / large / turbo
...
- 可调思考强度（自动 / 最小 / 低 / 中 / 高 / 极高）与 4000 字以内的角色设定
- 托盘常驻聊天窗口，文字与语音共享同一多轮上下文
- 会话侧栏支持新建与切换会话，消息支持上传图片和文件
...
| 诊断 | 音频采集、语音识别、大模型、语音合成、数据库与桌宠资源检查，导出脱敏诊断包 |
| 关于 | 版本信息与数据目录入口 |
...
内置两个形象：`dpsk-girl`（鲸鱼娘）与 `kawakaze`（江风 Q版）。
...
- 图集固定为 8 列 11 行，每格 192 × 208 像素，整图 1536 × 2288
- 行对应对待机、聆听、思考、执行等对话阶段，播放节奏由程序控制
- 可通过“导入形象包”（zip 压缩包）或“导入目录”安装，形象保存在 `%LOCALAPPDATA%\VoicePet\pets`
...
`config.json` 带版本号，字段不完整、包含未知项或类型错误时会回退到默认设置并给出提示。通过设置下载的识别模型统一保存在 `models/asr/<模型名>`，检测和加载使用同一目录，下载中断后可以重试。旧 Hugging Face 缓存不再自动使用；迁移并验证模型可用后，可以清理对应旧模型缓存。缓存可能被其他软件共享，程序不会自动删除它。`pets` 仅在导入形象时创建，`models/wake` 仅在实际下载或安装时创建。
